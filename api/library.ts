  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      return res.status(400).json({
        error: 'Unknown op',
        hint: 'sync | createFolder | renameFolder | deleteFolder | addFile | deleteFile | renameFile',
      });
        if (!ok) return res.status(404).json({ error: 'File not found' });
        return res.status(200).json({ success: true, ...dbStatus() });
      }
...
app.post('/api/library', express.json(), async (req, res) => {
  try {
    ...
    if (op === 'sync' || op === 'getLibrary') {
      // ... existing sync logic ...
      return res.status(200).json({ files: [], folders: [], ...dbStatus() });
    }

    if (op === 'createFolder') {
      // ... existing createFolder logic ...
      return res.status(200).json({ success: true, ...dbStatus() });
    }
